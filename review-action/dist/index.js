...
function validateConfig(config, repository) {
  ...
  for (const [index, lane] of config.lanes.entries()) {
    ...
    if (lane.review_prompt_suffix !== undefined && (typeof lane.review_prompt_suffix !== 'string' || !lane.review_prompt_suffix.trim())) throw new Error(`${location}.review_prompt_suffix must be a non-empty string when configured.`);
