...
| Lane | 当前供应商 | 协议 | 模型链 |
|---|---|---|---|
| A | 阿里 | OpenAI Chat Completions | Qwen3.8-Max → Qwen3.6-Flash |
...
- 模型的 `context_profile` 与 `max_output_tokens`：主 Qwen、GLM、Gemini 使用完整上下文；Lane A 的 Qwen3.6-Flash fallback 使用兼容的 `kimi-k3-throttled` 紧缩上下文配置，避免大 PR 再次耗尽模型时限。
...
- 主 Qwen、GLM、Gemini 保留完整审核上下文；三个业务仓维持 16384 输出上限。公开的 `ci-central` 元 CI 审核更容易触发长推理，因此仅其 Lane B 的 GLM/DeepSeek completion ceiling 为 32768，单请求/单模型预算分别为 10/12 分钟。
...
- `AI PR Review Retry` 监听失败的首轮 PR 审核。只有当前 head 的 Lane A 评论同时为 `diagnostic`、`endpoint-unavailable` 且包含 `fetch failed` 时，才调用 GitHub 的 failed-jobs rerun API；最多自动重跑一次。新 attempt 会分配新 runner，自动复用同 head/workflow 的 Lane B/C `valid` 证据。ci-central 自仓直接读取 `workflow_run`；业务仓薄 caller 通过 `workflow_call` 显式传递 run ID、attempt、head SHA 与 PR 号等非敏感元数据，中央 workflow 再用 GitHub API 读取权威 run 并逐项交叉校验。GitHub 的 failed-jobs API 会重跑该 run 的全部失败 job；同 head/workflow 的 `valid` Lane 会被证据去重逻辑复用。PR 已关闭、head 已变化、手动 `/review`、配额/鉴权/模型错误或第二次失败都不会继续重跑。
...
`probe-provider.ps1` 会依据中央配置选择协议，对 OpenAI 兼容端点先读取 `/models` 再发极小 Chat Completions 请求；Google Lane 发送 `generateContent` 请求并带上配置的 thinking level。Google probe 预留 512 completion tokens，避免高思考模式把过小预算全部耗在私有 reasoning 而没有最终 `OK`。脚本不输出 Key。
