...
- Qwen、DeepSeek、GLM 保留完整审核上下文。Lane B 的腾讯端点主模型为 `glm-5.3`；该模型官方最大输出为 128K tokens，reviewer 继续沿用已部署的 65536-token completion ceiling。同 Lane 备用 `deepseek-v4-pro-202606` 保留 393216 tokens 空间且不降低默认思考等级。两者各自使用 15 分钟 Lane 级请求/模型预算；Lane A 仍继承原有 5/6 分钟和 16384 输出上限。
